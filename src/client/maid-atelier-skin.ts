...
} from './maid-art/maid-atelier-chrome-art.generated.ts'
import {
 ...
 */
export function initMaidAtelier(): MaidSkinController {
  ...
  const dispose = (): void => {
    ...
    observer?.disconnect()
    if (observerFrame !== 0) cancelAnimationFrame(observerFrame)
    observerFrame = 0
    observerRecords = []
  ...
  let observerFrame = 0
  let observerRecords: MutationRecord[] = []

  const processObserverRecords = (records: readonly MutationRecord[]): void => {
  ...
  observer = new MutationObserver((records) => {
    for (const record of records) observerRecords.push(record)
    if (observerFrame !== 0) return
