...
function toTokens(id: string, p: DreamSkinPalette): ThemeTokens {
  ...
  const base = wallpaper !== undefined
    ? `linear-gradient(90deg, ${hexToRgba(p.background, 0.94)} 0%, ${hexToRgba(p.background, 0.72)} 50%, ${hexToRgba(p.background, 0.18)} 84%, transparent 100%), url("${wallpaper}") center/cover no-repeat`
    : p.background
  return {
...
export const DREAM_SKIN_PRESETS: readonly DreamSkinPreset[] = Object.freeze([
  // ── Built-in Codex-Dream-Skin presets ─────────────────────────────────────
  ...
  // ── dreamskin.cc community gallery, top 30 by downloads ──────────────────
