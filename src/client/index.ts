...
import { initMaidSkin } from './maid-skin.ts'
...
import {
  ...
  writeCursorUploads,
  type CursorSkinId,
} from './cursor-images.ts'
import { initSettingsPerf } from './settings-perf.ts'
 ...
 */
const WE_OVERRIDDEN_TOKENS = new Set<string>([
  '--dsw-alias-bg-base',
...
interface StoredPrefs {
  ...
  /** Whale cursor master switch; off falls back to the native OS cursor. */
  cursorEnabled: boolean
  /** Whale cursor art skin; `whale` built-in, `custom` user-authored. */
  cursorSkin: CursorSkinId
  /** Whale cursor render size in px (24..64). */
  cursorSize: number
  /** Per-state enable map; false keeps the native cursor for that state. */
 ...
 * and fails closed.
 */
function initSubagentWatcher(ctx: ClientContext): () => void {
  ...
  const readIds = (): string[] => {
    try {
      const snap = list.getSnapshot() as {
        current?: string
        subagentsByParent?: Record<string, { entries?: Array<{ kind?: string; id?: string }> }>
      }
...
function removeThemeWallpaperLayer(): void {
  if (themeWallpaperEl !== null) {
...
function shiftLuma(hex: string): number {
  const h = hex.replace('#', '')
  const full = h.length === 3 ? h.split('').map(c => c + c).join('') : h
  const num = Number.parseInt(full, 16)
  if (full.length !== 6 || Number.isNaN(num)) return 0.5
  const r = (num >> 16) & 255
  const g = (num >> 8) & 255
  const b = num & 255
...
function applySkin(themeId: string, scrimStrength: number, custom?: CustomThemeInput): void {
  ...
  const clear = (): void => {
    for (const token of SKIN_TOKENS) {
      if (weActive && WE_OVERRIDDEN_TOKENS.has(token)) continue
  ...
  const focus: Partial<WallpaperTweak> = {}
...
export function apply(ctx: ClientContext): void {
  ...
  // host dark theme.
  let maid: MaidSkinController | undefined
  let atelier: MaidSkinController | undefined
  const syncMaidSkin = (themeId: string): void => {
  ...
  const injected = (actions: BoundActions<typeof store>): DreamSkinInjected => {
    ...
    return {
      ...
      setCursorStateOverride: (state: string, on: boolean) => {
      ...
      setCursorEnabled: (on: boolean) => {
        const next = readPrefs()
        next.cursorEnabled = on
        // 「默认箭头」是光标的基础形态：若它在状态开关里被关闭，总开关
        // 打开后皮肤永远无法显示（gate 始终关闭），看起来就像"开关失效"。
        // 打开总开关时自动恢复它，保证启用即有皮肤。
        if (on && next.cursorStateOverrides.default === false) {
          delete next.cursorStateOverrides.default
        }
        writePrefs(next)
        cursorEnabled = on
        bound?.syncCursorEnabled(on)
        if (on) {
          bound?.syncCursorStateOverrides(next.cursorStateOverrides)
          cursor.setStateOverrides(next.cursorStateOverrides)
          // 打开开关的瞬间就回到默认形态并强制重绘，皮肤立即可见
          // （不依赖下一次 pointermove 的检测时序）。
          cursor.refresh()
      ...
      tweak: (partial: Partial<WallpaperKnobs>) => {
