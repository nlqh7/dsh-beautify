 ...
 */
import { createFrameController } from './maid-art/frames.ts'
import { createOrnamentController } from './maid-art/ornaments.ts'
import { ILLUSTRATED_BACKGROUND, SIDEBAR_OCEAN_BACKGROUND } from './maid-art/background-art.generated.ts'
import { PAPER_BACKDROP_DARK, PAPER_BACKDROP_LIGHT, PET_ART } from './maid-art/art.ts'
 ...
 */
export function initMaidSkin(): MaidSkinController {
  ...
  let chromeFrame = 0
  const queueSyncChrome = (): void => {
    if (chromeFrame !== 0) return
  ...
  return {
    ...
    dispose() {
      ...
      if (chromeFrame !== 0) { cancelAnimationFrame(chromeFrame); chromeFrame = 0 }
