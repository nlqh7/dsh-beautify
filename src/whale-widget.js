...
function layoutBabies() {
  ...
  list.forEach(function (id) {
    ...
    // 从屏幕最左依次向右（autoIndex 0 在最左，依次右移），最后一只不超过主鲸鱼左侧
    var x = 8 + autoIndex * (size + gap)
    // 上限：不超过主鲸鱼左侧（留 gap 间距）——若超过就让它贴主鲸鱼左侧
    if (x + size > r.left - gap) x = Math.max(8, r.left - size - gap)
    x = clamp(x, 6, Math.max(6, vp.w - size - 6))
