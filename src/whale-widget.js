...
function layoutBabies() {
  ...
  list.forEach(function (id) {
    ...
    // 默认：从主鲸鱼**左侧**紧邻向右排（避开 dsh 侧边栏 ~260px）
    // 第一只紧贴主鲸鱼左缘-gap，后续依次右移；最右不超过侧边栏右缘
    var sidebarW = 260
    var baseX = Math.max(r.left - size - gap, sidebarW + gap)
    if (baseX + size > vp.w - 6) baseX = Math.max(sidebarW + gap, vp.w - size - 6)
    var x = baseX + autoIndex * (size + gap)
    x = clamp(x, Math.max(6, sidebarW + gap - 20), Math.max(6, vp.w - size - 6))
