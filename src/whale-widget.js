...
// 反应 GIF 素材（蓝色大肥鱼表情包，用户自选放入 assets/reactions/）
const ANGRY_GIF_CANDIDATES = [
  path.join(PACKAGE_ROOT, 'assets', 'reactions', 'angry.gif'),
]
const PAT_GIF_CANDIDATES = [
  path.join(PACKAGE_ROOT, 'assets', 'reactions', 'pat.gif'),
]
const IDLE_GIF_CANDIDATES = [
  path.join(PACKAGE_ROOT, 'assets', 'reactions', 'idle.gif'),
]
...
function apply(ctx) {
    ...
    function loadAngryGif() {
      ...
      for (const p of ANGRY_GIF_CANDIDATES) {
        try {
          const bytes = fs.readFileSync(p)
          if (bytes && bytes.length > 0) {
            angryGifBytes = bytes
    ...
    function loadPatGif() {
      ...
      for (const p of PAT_GIF_CANDIDATES) {
        try {
          const bytes = fs.readFileSync(p)
          if (bytes && bytes.length > 0) {
            patGifBytes = bytes
    ...
    function loadIdleGif() {
      ...
      for (const p of IDLE_GIF_CANDIDATES) {
        try {
          const bytes = fs.readFileSync(p)
          if (bytes && bytes.length > 0) {
            idleGifBytes = bytes
