...
var css = [
  ...
  '.dshwv-babies{position:absolute;left:0;top:0;width:100%;height:100%;pointer-events:none;z-index:0}',
  '.dshwv-baby{position:absolute;bottom:2%;width:calc(var(--dshw-base) * 0.17);height:calc(var(--dshw-base) * 0.17);display:block;pointer-events:none;-webkit-user-drag:none;user-select:none;opacity:0;transform:translateY(26%) scale(.5);transition:opacity .28s ease,transform .35s cubic-bezier(.34,1.56,.64,1);image-rendering:-webkit-optimize-contrast}',
...
var babySeq = 0
// 按当前子代理顺序重排所有小小鲸鱼的位置，个体之间留出明显间隔。
function layoutBabies() {
  var list = Array.from(babyWhales.keys())
  var count = list.length
  var size = count > 5 ? 0.14 : 0.16
  var gap = count > 5 ? 0.014 : 0.03
  list.forEach(function (id, index) {
    var el = babyWhales.get(id)
    if (!el) return
    el.style.left = (1.5 + index * (size * 100 + gap * 100)) + '%'
  })
}
