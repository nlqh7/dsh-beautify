	...
	function buildInventory() {
		...
		const wallpapers = all.map((w) => {
			...
			return {
				...
				type: w.type,
				playable: hasMedia,
				media: hasMedia ? w.type === "web" ? `${BASE}/media/${tokenFor(w.dir)}/` : `${BASE}/media/${tokenFor(w.fileAbs)}` : null,
	...
	for (const seg of ["media", "preview"]) {
		...
		disposers.push(webServer.register({
			...
			handler: (req, res) => {
				...
				serveFile(resolveMediaToken(decodeURIComponent(pathname.slice(prefix.length))), req, res);
			}
		}));
	}
	return () => {
...
const ANGRY_GIF_CANDIDATES = [path.join(PACKAGE_ROOT, "assets", "reactions", "angry.gif")];
const PAT_GIF_CANDIDATES = [path.join(PACKAGE_ROOT, "assets", "reactions", "pat.gif")];
const IDLE_GIF_CANDIDATES = [path.join(PACKAGE_ROOT, "assets", "reactions", "idle.gif")];
...
// 挂件双形象：'dsniang'（默认，静态立绘+Q弹）| 'dafeiyu'（大肥鱼表情包形象，
// 本体为 GIF 动画，摸头/生气切换对应反应图）。键与美化设置面板共享。
var SKIN_STORAGE_KEY = 'dsh-beautify:whaleSkin'
var IDLE_GIF_URL = '/dsh-whale/reactions/idle.gif'
var ANGRY_GIF_URL = '/dsh-whale/reactions/angry.gif'
var PAT_GIF_URL = '/dsh-whale/reactions/pat.gif'
var REACTION_FALLBACK_MS = 3000
var MIN_REACTION_MS = 800
var SEQ_TAIL_MS = 300
var angrySeqToken = null
function readWhaleSkin() {
  try { return localStorage.getItem(SKIN_STORAGE_KEY) === 'dafeiyu' ? 'dafeiyu' : 'dsniang' } catch (err) { return 'dsniang' }
}
function baseImgUrl() { return readWhaleSkin() === 'dafeiyu' ? IDLE_GIF_URL : IMG_URL }
...
var css = [
  ...
  '.dshwv-menu{position:fixed;min-width:196px;background:rgba(255,255,255,.86);backdrop-filter:blur(14px) saturate(1.5);-webkit-backdrop-filter:blur(14px) saturate(1.5);border:1px solid rgba(32,49,112,.22);border-radius:14px;padding:12px 14px;opacity:0;transform:scale(.92) translateY(-4px);transform-origin:top right;transition:opacity .18s ease,transform .2s cubic-bezier(.34,1.56,.64,1);pointer-events:none;z-index:10000;box-shadow:0 12px 32px rgba(32,49,112,.20),0 2px 8px rgba(32,49,112,.10);color-scheme:light}',
  '.dshwv-menu.dshwv-menu-open{opacity:1;transform:scale(1) translateY(0);pointer-events:auto}',
  '.dshwv-menu-row{display:flex;align-items:center;gap:8px;margin:4px -6px;padding:3px 6px;border-radius:8px;color:#203170;font-size:12px;white-space:nowrap;transition:background .15s ease}',
  '.dshwv-menu-row:hover{background:rgba(32,49,112,.07)}',
  '.dshwv-range{flex:1;min-width:0;accent-color:#203170}',
  '.dshwv-number{width:44px;border:1px solid rgba(32,49,112,.35);border-radius:8px;padding:2px 4px;font-size:12px;color:#203170;background:#fff;box-sizing:border-box;transition:border-color .15s ease,background .15s ease}',
  '.dshwv-number:focus{outline:none;border-color:rgba(32,49,112,.7)}',
  '.dshwv-number:disabled{opacity:.4;background:rgba(32,49,112,.06);cursor:not-allowed}',
  '.dshwv-sound{flex:1;border:1px solid rgba(32,49,112,.35);border-radius:8px;background:rgba(32,49,112,.08);color:#203170;font-size:12px;padding:3px 0;cursor:pointer;transition:background .15s ease,border-color .15s ease}',
  ...
  '.dshwv-menu-sep{height:1px;background:linear-gradient(90deg,transparent,rgba(32,49,112,.28),transparent);margin:7px 0}',
  '.dshwv-volpct{width:44px;text-align:right;color:#203170;font-size:12px;font-variant-numeric:tabular-nums}',
  ...
  '.dshwv-baby-speech{position:fixed;z-index:10001;background:rgba(255,255,255,.92);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);color:#203170;padding:5px 12px;border-radius:14px;border:1px solid rgba(32,49,112,.18);font-size:13px;font-weight:600;line-height:1.2;box-shadow:0 6px 18px rgba(32,49,112,.16);pointer-events:none;transform:translateY(6px) scale(.82);opacity:0;transition:opacity .2s ease,transform .32s cubic-bezier(.34,1.56,.64,1);white-space:nowrap}',
...
img.src = baseImgUrl()
// 美化设置面板切换挂件形象时实时跟随（同页签直接监听自定义事件，跨页签走 storage）
try {
  window.addEventListener('storage', function (e) {
    if (e.key === SKIN_STORAGE_KEY && !angrySeqToken) img.src = baseImgUrl()
  })
  window.addEventListener('dshw-skin-change', function () {
    if (!angrySeqToken) img.src = baseImgUrl()
  })
} catch (err) {}
img.alt = 'DeepSeek 余额'
img.draggable = false
...
function applyBubbleLines(lines) {
  if (lines && lines.gif) {
    ...
    // lines.hint 可选：gif 下方保留一行说明文字（生气气话等）。
    if (gifFailed) {
      ...
      if (lines.hint) {
        ...
        hintEl.className = 'dshwv-label'
        hintEl.textContent = lines.hint
        hintEl.style.color = lines.hintColor || ''
      } else {
...
// ---- 连戳生气状态机（阈值与衰减对齐 pet-whale 的 pokeStreak 设计） ----
var POKE_SULK_AT = 6      // 戳到第 6 下：生气（播放 GIF + 气话）
var POKE_DECAY_MS = 2600  // 停手这么久，连戳计数清零
var SULK_MS = 4200        // 闹脾气持续时长，期间再戳只会换气话
var petStreak = 0          // 当前连戳次数
var petDecayTimer = 0      // 衰减计时器
var petSulking = false     // 闹脾气中标志
var petSulkTimer = 0       // 闹脾气到期计时器
function bumpPetStreak() {
  petStreak += 1
  if (petDecayTimer !== 0) { clearTimeout(petDecayTimer); petDecayTimer = 0 }
  petDecayTimer = setTimeout(function () { petDecayTimer = 0; petStreak = 0 }, POKE_DECAY_MS)
}
function clearPetSulk() {
  if (petSulkTimer !== 0) { clearTimeout(petSulkTimer); petSulkTimer = 0 }
  petSulking = false
}
function triggerSulk() {
  clearPetSulk()
  petSulking = true
  if (costBubbleActive) { if (costBubbleTimer) { clearTimeout(costBubbleTimer); costBubbleTimer = null } costBubbleActive = false }
  if (readWhaleSkin() === 'dafeiyu') playAngryGif(function () {})
  showAngryBubble()
  petSulkTimer = setTimeout(function () { petSulkTimer = 0; clearPetSulk(); petStreak = 0 }, SULK_MS)
}
// 生气台词气泡：走 showBubble() 的完整开框流程保证聊天框正常，再把内容换成气话
function showAngryBubble() {
  if (!bubbleOn) return
  showBubble()
  bubbleRandomActive = true
  bubbleRandomLines = { gif: true, hint: pickOne(ANGRY_LINES), hintColor: '#e0433f' }
  swapBubbleContent(function () { applyBubbleLines(bubbleRandomLines) })
}
...
function showCostBubble(amount) {
  ...
  if (bubbleTimer) { clearTimeout(bubbleTimer); bubbleTimer = null }
  if (gifFadeTimer) { clearTimeout(gifFadeTimer); gifFadeTimer = null }
  // 挂起中的台词切换也要取消：否则点气泡后 190ms 内一轮对话结束，
  // 迟到的 applyFn 会用随机台词（甚至 gif）覆盖刚写好的消耗金额。
  if (bubbleSwapTimer) { clearTimeout(bubbleSwapTimer); bubbleSwapTimer = null }
...
function express() {
  ...
  // 主鲸鱼移动/翻转时让子代理小鲸鱼跟着重排；但**拖动中**不重排——
  // 否则每 move 一次小鲸鱼就跳一次。松手后 endDrag → settle() → express() 重排归位。
  if (!drag || !drag.active) {
    layoutBabies()
  }
}
...
// 解析 GIF 总时长：累加每帧 Graphic Control Extension 延迟（移植自大肥鱼桌宠）。
// 结构不可读或无帧延迟时返回 fallbackMs。
function parseGifDurationMs(buf, fallbackMs) {
  try {
    ...
    var packed = dv.getUint8(10)
    var pos = 13
    if (packed & 0x80) pos += 3 * (2 << (packed & 0x07))
    var totalMs = 0
    while (pos + 1 <= buf.byteLength) {
      var b = dv.getUint8(pos)
      if (b === 0x3b) break
      if (b === 0x21) {
        var label = dv.getUint8(pos + 1)
        pos += 2
        if (label === 0xf9) {
          var size = dv.getUint8(pos)
          if (size !== 4 || pos + size + 2 > buf.byteLength) return fallbackMs
          totalMs += dv.getUint16(pos + 1, true) * 10
          pos += size + 1
          continue
        }
        while (pos < buf.byteLength) {
          var sz = dv.getUint8(pos); pos += 1
          if (sz === 0) break
          pos += sz
        }
        continue
      ...
      if (b === 0x2c) {
        ...
        var flags = dv.getUint8(pos + 9)
        pos += 10
        if (flags & 0x80) pos += 3 * (2 << (flags & 0x07))
        if (pos >= buf.byteLength) return fallbackMs
        var lzwMin = dv.getUint8(pos); pos += 1
        if (lzwMin > 11) return fallbackMs
        while (pos < buf.byteLength) {
          var bsz = dv.getUint8(pos); pos += 1
          if (bsz === 0) break
          pos += bsz
        }
        continue
  ...
  } catch (err) { return fallbackMs }
}

// 播放反应 GIF 一遍后回到当前形象的默认图；GIF 缺失/加载失败时 done(false)。
function playReactionGif(url, done) {
  try {
    if (angrySeqToken) angrySeqToken.cancelled = true
    ...
    fetch(url, { cache: 'force-cache' }).then(function (res) {
      ...
      if (token.cancelled) { if (done) done(false); return }
      var dur = parseGifDurationMs(buf, REACTION_FALLBACK_MS)
      var blobUrl = URL.createObjectURL(new Blob([buf], { type: 'image/gif' }))
      img.src = blobUrl
      setTimeout(function () {
        ...
        angrySeqToken = null
        if (done) done(true)
      }, Math.max(dur, MIN_REACTION_MS) + SEQ_TAIL_MS)
    }).catch(function () {
      angrySeqToken = null
      if (done) done(false)
    })
...
function playAngryGif(done) { playReactionGif(ANGRY_GIF_URL, done) }
function playPatGif(done) { playReactionGif(PAT_GIF_URL, done) }
...
// 摸头生气：PET_WINDOW_MS 内按压满 PET_ANGRY_COUNT 次触发一次，ANGRY_COOLDOWN_MS 内不再累计
var petStamps = []
var angryUntil = 0
var PET_WINDOW_MS = 10000
var PET_ANGRY_COUNT = 8
var ANGRY_COOLDOWN_MS = 15000
var ANGRY_LINES = [
  '别摸啦！头要秃了！',
  '哼！生气了！不理你了！',
  '再摸就要收费了！一次一条鱼！',
...
// 摸头计数：滑动窗口内按压达到阈值 → getAngry()。冷却期内不累计。
function notePet() {
  var now = Date.now()
  if (now < angryUntil) return
  while (petStamps.length && now - petStamps[0] > PET_WINDOW_MS) petStamps.shift()
  petStamps.push(now)
  if (petStamps.length >= PET_ANGRY_COUNT) {
    petStamps.length = 0
    getAngry()
  }
}
// 生气：左右抖动 + 气泡弹出气话。台词走 bubbleRandomLines 承载，
// 复用「再点气泡关闭」与 render() 刷新时不覆盖随机台词的既有路径；
// 气泡关闭后 hideBubble 清态，下次 showBubble 自然恢复余额显示。
// 气泡总开关关闭时只抖动不说话；消耗泡泡显示中触发生气则先清掉它
// （用户主动交互优先，否则两套泡泡状态互踩：点击关闭/计时器互相截断）。
function getAngry() {
  angryUntil = Date.now() + ANGRY_COOLDOWN_MS
  if (costBubbleActive) {
    if (costBubbleTimer) { clearTimeout(costBubbleTimer); costBubbleTimer = null }
    costBubbleActive = false
  }
...
function snapCheck() {
  ...
  var centerY = top + h / 2
  var moved = false
  // 吸附阈值 = 自身大小：离墙 ≤ 自身半宽时贴墙（小东西紧贴就吸，大东西半宽内才吸）
  var snap = Math.max(w, h)
  if (centerX <= snap) {
    state.h = 'left'
    state.hOff = 0
    left = 0
    moved = true
  } else if (centerX >= vp.w - snap) {
    state.h = 'right'
    state.hOff = 0
...
function endDrag(e, clickAllowed) {
  ...
  if (clickAllowed && !drag.moved) {
    ...
    if (petStreak >= POKE_SULK_AT) {
      triggerSulk()
      return
    }
    // 正常摸头：本体播脸红爱心 GIF（素材缺失则跳过），气泡照常
    if (readWhaleSkin() === 'dafeiyu') playPatGif(function () {})
  ...
  // 吸附阈值 = 自身大小（与 snapCheck 一致的手感）
  var snap = Math.max(drag.w, drag.h)
  if (centerX <= snap) {
    state.h = 'left'
    state.hOff = 0
  } else if (centerX >= drag.vp.w - snap) {
    state.h = 'right'
    state.hOff = 0
  } else {
    state.h = null
  ...
  if (centerY <= snap) {
    state.v = 'top'
    state.vOff = 0
  } else if (centerY >= drag.vp.h - snap) {
    state.v = 'bottom'
    state.vOff = 0
  } else {
    state.v = null
...
// 坐标系纪律：入参/返回值/位移全部是**图片矩形**坐标；碰撞箱只用于
// 相交测试（此前把碰撞箱原点当返回值，小鲸鱼每次拖动/重排都向左上漂移）。
  ...
  try {
    ...
    for (var pass = 0; pass < 8; pass++) {
      ...
      for (var i = 0; i < obstacles.length; i++) {
        var ob = obstacles[i]
        if (ml < ob.r - pad && ml + mw > ob.l + pad && mt < ob.b - pad && mt + mh > ob.t + pad) {
          o = ob
          break
        }
      }
      ...
      var dLeft = ml + mw - o.l + pad
      var dRight = o.r - ml + pad
      var dUp = mt + mh - o.t + pad
      var dDown = o.b - mt + pad
      var options = [
      ...
      for (var k = 0; k < options.length; k++) {
        // 位移应用回图片坐标并 clamp（防推出后图片出屏）；碰撞箱跟随平移
        var nl = clamp(l0 + options[k].dl, 0, Math.max(0, vp.w - w))
        var nt = clamp(t0 + options[k].dt, 0, Math.max(0, vp.h - h))
        if (nl !== l0 || nt !== t0) {
          ml += nl - l0
          mt += nt - t0
          l0 = nl
    ...
    l0 = clamp(l0, 0, Math.max(0, vp.w - w))
    t0 = clamp(t0, 0, Math.max(0, vp.h - h))
    return { left: Math.round(l0), top: Math.round(t0) }
  } catch (err) {
    return { left: l, top: t }
  }
}
// 默认位置：紧贴主鲸鱼图片底部水平排成一排（依次向右），最右一只
// 紧贴主鲸鱼左侧不重叠。这样多只小鲸鱼默认不相互碰撞、不遮挡 dsh
// 聊天框/输入区（贴在屏幕底部，远离中央对话区），也不盖主鲸鱼
// 自己的余额气泡（在主鲸鱼图片内）。手动拖走的保持用户位置但不
// 侵入主鲸鱼（pushOut 推出）。
function layoutBabies() {
  ...
  var gap = 10
  // 默认：紧贴主鲸鱼底部水平排
  var y = clamp(r.bottom - size, 6, Math.max(6, vp.h - size - 6))
  var autoIndex = 0
  list.forEach(function (id) {
    ...
    // 默认：固定从**左侧边栏(260px)右缘**开始向右排，紧贴主鲸鱼底部——
    // 不跟着主鲸鱼左缘跑（主鲸鱼在屏幕左边时小鲸鱼也不会钻进侧边栏）。
    var baseX = 268
    var x = baseX + autoIndex * (size + gap)
    x = clamp(x, baseX, Math.max(baseX, vp.w - size - 6))
...
// 孵化定时器的守卫读模块级最新集合，而不是本次事件的 alive 快照——
// 否则快速起停（<n×170ms）时过期定时器会把已结束的子代理孵成孤儿小鲸鱼。
var latestBabyIds = []
function syncBabyWhales(ids) {
  ...
  added.forEach(function (id, i) {
    setTimeout(function () {
      if (latestBabyIds.indexOf(id) < 0) return
      spawnBabyWhale(id)
...
function endBabyDrag() {
  ...
  if (el) {
    ...
    if (babyDrag.moved) {
      ...
      // 吸附阈值 = 自身大小：离墙 ≤ 自身半宽时贴墙
      var cx = l + w / 2
      var cy = t + h / 2
      var snap = Math.max(w, h)
      if (cx <= snap) l = 0
      else if (cx >= vp.w - snap) l = Math.max(0, vp.w - w)
      if (cy <= snap) t = 0
      else if (cy >= vp.h - snap) t = Math.max(0, vp.h - h)
...
function apply$1(ctx) {
	...
	let gifBytes = null;
	let angryGifBytes = null;
	let patGifBytes = null;
	let idleGifBytes = null;
	let turnAggs = /* @__PURE__ */ new Map();
	let lastTurn = null;
	let lastTurnSeq = 0;
	const disposers = [];
	...
	function handleSessionEvent(sessionId, event) {
		try {
			...
			const p = priceFor(d.message && d.message.source ? d.message.source.model : "");
			const off = isPeakTime(Math.floor(Date.now() / 1e3)) ? 1 : 0;
	...
	function loadGif() {
		...
		for (const p of RUA_GIF_CANDIDATES) try {
	...
	function loadAngryGif() {
		if (angryGifBytes) return angryGifBytes;
		for (const p of ANGRY_GIF_CANDIDATES) try {
			const bytes = fs.readFileSync(p);
			if (bytes && bytes.length > 0) {
				angryGifBytes = bytes;
				return bytes;
		...
		throw new Error("angry gif not found");
	}
	function loadPatGif() {
		if (patGifBytes) return patGifBytes;
		for (const p of PAT_GIF_CANDIDATES) try {
			const bytes = fs.readFileSync(p);
			if (bytes && bytes.length > 0) {
				patGifBytes = bytes;
				return bytes;
		...
		throw new Error("pat gif not found");
	}
	function loadIdleGif() {
		if (idleGifBytes) return idleGifBytes;
		for (const p of IDLE_GIF_CANDIDATES) try {
			const bytes = fs.readFileSync(p);
			if (bytes && bytes.length > 0) {
				idleGifBytes = bytes;
				return bytes;
		...
		throw new Error("idle gif not found");
	}
	function loadImage() {
	...
	function pickBalanceInfo(infos) {
		...
		const num = (x) => x && x.total_balance !== void 0 ? Number(x.total_balance) : NaN;
		return infos.find((x) => x && x.currency === "CNY" && num(x) > 0) || infos.find((x) => num(x) > 0) || infos.find((x) => x && x.currency === "CNY") || infos[0];
	}
	...
	function readSizeConfig() {
		for (const p of SIZE_FILE_CANDIDATES) try {
			...
			if (parsed && typeof parsed.scale === "number") return {
				...
				turnCostCloseMs: typeof parsed.turnCostCloseMs === "number" ? parsed.turnCostCloseMs : 5e3,
	...
	function writeSizeConfig(scale, sound, vol, soundSet, usageMode, peakMode, bubbleOn, turnCostOn, turnCostCloseMs, scrollGapOn, scrollGapPx, babySound) {
		...
		const tcc = typeof turnCostCloseMs === "number" ? turnCostCloseMs > 0 ? turnCostCloseMs : 0 : 5e3;
	...
	disposers.push(ctx.webServer.register({
		kind: "exact",
		path: "/dsh-whale/reactions/angry.gif",
		handler: (req, res) => {
			try {
				const bytes = loadAngryGif();
				res.writeHead(200, {
					"Content-Type": "image/gif",
					"Cache-Control": "no-store",
					"Content-Length": String(bytes.length)
				...
				res.writeHead(404, { "Content-Type": "text/plain; charset=utf-8" });
				res.end("angry gif unavailable: " + String(err && err.message || err));
			}
		}
	}));
	disposers.push(ctx.webServer.register({
		kind: "exact",
		path: "/dsh-whale/reactions/pat.gif",
		handler: (req, res) => {
			try {
				const bytes = loadPatGif();
				res.writeHead(200, {
					"Content-Type": "image/gif",
					"Cache-Control": "no-store",
					"Content-Length": String(bytes.length)
				...
				res.writeHead(404, { "Content-Type": "text/plain; charset=utf-8" });
				res.end("pat gif unavailable: " + String(err && err.message || err));
			}
		}
	}));
	disposers.push(ctx.webServer.register({
		kind: "exact",
		path: "/dsh-whale/reactions/idle.gif",
		handler: (req, res) => {
			try {
				const bytes = loadIdleGif();
				res.writeHead(200, {
					"Content-Type": "image/gif",
				...
				res.writeHead(404, { "Content-Type": "text/plain; charset=utf-8" });
				res.end("idle gif unavailable: " + String(err && err.message || err));
			}
		}
	}));
	disposers.push(ctx.webServer.register({
		...
		handler: async (req, res) => {
			if (req.method === "PUT" || req.method === "POST") {
				try {
					...
					const result = writeSizeConfig(scale, parsed.sound !== false, parsed.vol, parsed.soundSet, parsed.usageMode, parsed.peakMode, parsed.bubbleOn, parsed.turnCostOn, parsed.turnCostCloseMs, parsed.scrollGapOn, parsed.scrollGapPx, parsed.babySound !== false);
