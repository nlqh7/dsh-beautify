...
document.addEventListener('pointermove', function (e) {
  var whaleHit = isWhaleHit(e)
  var babyEl = isBabyHit(e)
  var hit = whaleHit || babyEl !== null
  if (hit === lastWhaleHit) return
  lastWhaleHit = hit
  try {
    if (hit) {
      document.documentElement.setAttribute('data-dsh-whale-hover', '')
      // 摸头回弹：进入瞬间给命中的小鲸鱼（主鲸鱼或子代理）弹一下。
      var target = babyEl || document.querySelector('.dshwv-img')
      if (target) {
        target.classList.remove('dshwv-petting', 'dshwv-baby-petting')
        // 强制 reflow，保证连续多次摸头也能重新触发动画
        void target.offsetWidth
        target.classList.add(babyEl ? 'dshwv-baby-petting' : 'dshwv-petting')
        setTimeout(function () {
          try { target.classList.remove('dshwv-petting', 'dshwv-baby-petting') } catch (err) {}
        }, 480)
      }
    } else {
      document.documentElement.removeAttribute('data-dsh-whale-hover')
    }
  } catch (err) {}
}, { passive: true })
