...
// 主鲸鱼墙：小鲸鱼不能侵入主鲸鱼矩形——拖动/重排时把中心钳制在主鲸鱼
// 扩大（半宽 + 间距）的矩形之外。中心一旦进入"墙内"就推到最近墙，
// 指针继续移动不会翻转（卡左墙就一直是左墙，绕过主鲸鱼才能到另一边）。
function clampAgainstMain(l, t, w, h) {
  try {
    var rr = root.getBoundingClientRect()
    if (!rr || rr.width <= 0 || rr.height <= 0) return { left: l, top: t }
    var pad = 6
    var cx = l + w / 2
    var cy = t + h / 2
    var leftWall = rr.left - pad - w / 2
    var rightWall = rr.right + pad + w / 2
    var topWall = rr.top - pad - h / 2
    var bottomWall = rr.bottom + pad + h / 2
    if (cx > leftWall && cx < rightWall) {
      cx = (cx - rr.left) <= (rr.right - cx) ? leftWall : rightWall
    }
    if (cy > topWall && cy < bottomWall) {
      cy = (cy - rr.top) <= (rr.bottom - cy) ? topWall : bottomWall
    }
    l = cx - w / 2
    t = cy - h / 2
    var vp = viewport()
...
function layoutBabies() {
  ...
  list.forEach(function (id) {
    ...
    if (babyManual.has(id)) {
      ...
      var walled = clampAgainstMain(curL, curT, cw, ch)
      if (walled.left !== curL || walled.top !== curT) {
        el.style.left = walled.left + 'px'
        el.style.top = walled.top + 'px'
      }
      return
    }
...
function onBabyPointerMove(e) {
  ...
  // 主鲸鱼墙：拖动中实时撞墙（不侵入主鲸鱼矩形），松手才不会乱弹
  var walled = clampAgainstMain(nx, ny, w, h)
  el.style.left = walled.left + 'px'
  el.style.top = walled.top + 'px'
}
