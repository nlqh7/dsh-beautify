...
function emit() { for (const fn of [...listeners])
    fn(); }
...
export async function loadInventory() {
    ...
    if (selection.rotationEnabled) {
        if (!selection.rotationGroupId) {
            ...
            if (usable)
                selection.rotationGroupId = usable.id;
            else
                selection.rotationEnabled = false;
        }
        else if (rotationCandidates().length < 2) {
            const usable = firstUsableGroup();
            if (usable && usable.id !== selection.rotationGroupId)
                selection.rotationGroupId = usable.id;
            else if (!usable)
                selection.rotationEnabled = false;
        }
...
function groupWallpapers(group) {
    if (!group || !Array.isArray(group.wallpaperIds))
        return [];
...
function clearRotationTimer() {
    if (selection.rotationTimer === null)
        return;
...
function syncRotationTimer() {
    clearRotationTimer();
    if (!selection.rotationEnabled || !selection.id)
        return;
    if (rotationCandidates().length < 2)
        return;
...
function saveEditingGroup() {
    ...
    if (idx >= 0)
        selection.rotationGroups[idx] = cleaned;
    else
        selection.rotationGroups.push(cleaned);
...
function deleteGroup(id) {
    ...
    if (selection.rotationGroupId === id) {
        ...
        if (selection.rotationEnabled) {
            ...
            if (fallback)
                selection.rotationGroupId = fallback.id;
            else
                selection.rotationEnabled = false;
        }
    }
    if (selection.editing && selection.editing.id === id)
        selection.editing = null;
    persistSelection();
    syncRotationTimer();
    emit();
...
function importPlaylistIntoDraft(playlist) {
    if (!selection.editing || !playlist || !Array.isArray(playlist.wallpaperIds))
        return;
...
export function setWeEffect(kind, value) {
    if (kind === 'scrim')
        selection.scrim = value / 100;
    else if (kind === 'border')
        selection.border = value / 100;
    else if (kind === 'blur')
        selection.blur = value;
    else if (kind === 'wallpaperBlur')
        selection.wallpaperBlur = value;
    persistSelection();
    applyEffects();
    emit();
}
...
export function setWallpaper(partial) {
    ...
    try {
        localStorage.setItem(WALLPAPER_KEY, JSON.stringify(w));
    }
    catch { /* ignore */ }
    emit();
}
...
function restoreMaidTheme() {
    ...
    if (maidVarStash) {
        const css = document.body.style;
        for (const [n, v] of Object.entries(maidVarStash))
            css.setProperty(n, v);
        maidVarStash = null;
    }
}
...
export function readGlass() {
    try {
        ...
        if (raw === null) {
            ...
            try {
                localStorage.setItem(GLASS_KEY, JSON.stringify(migrated));
            }
            catch { /* ignore */ }
            return migrated;
        }
...
export function setGlass(kind, value) {
    ...
    g[kind] = value;
    try {
        localStorage.setItem(GLASS_KEY, JSON.stringify(g));
    }
    catch { /* ignore */ }
    applyGlass();
    emit();
...
function ensureGlassStyle() {
    if (glassStyleEl && document.head.contains(glassStyleEl))
        return glassStyleEl;
    if (glassStyleEl && !document.head.contains(glassStyleEl)) {
        glassStyleEl.remove();
        glassStyleEl = null;
...
function SliderRow(label, min, max, step, value, onInput, suffix) {
    return React.createElement("div", { className: "we-picker__row we-picker__slider-row" }, React.createElement("span", { className: "we-picker__hint we-picker__label" }, label), React.createElement("input", {
...
function WallpaperPicker() {
    ...
    const onDeleteGroup = () => {
        ...
        if (typeof window !== "undefined" && typeof window.confirm === "function") {
            if (!window.confirm("删除轮播列表「" + group.name + "」？"))
                return;
        }
    ...
    if (!sel.loaded) {
        return React.createElement("div", { className: "we-picker" }, React.createElement("span", { className: "we-picker__hint" }, "扫描 Wallpaper Engine…"));
    }
    if (sel.inventory.error) {
        return React.createElement("div", { className: "we-picker" }, React.createElement("div", { className: "we-picker__error" }, "未检测到 Wallpaper Engine：" + sel.inventory.error), React.createElement("button", {
    ...
    return React.createElement("div", { className: "we-picker" }, React.createElement("select", { className: "we-picker__select", value: sel.id, onChange }, React.createElement("option", { value: "" }, "— 无（关闭） —"), ...list.map((w) => React.createElement("option", {
        key: w.id, value: w.id, disabled: !isRotatableWallpaper(w),
    }, (isRotatableWallpaper(w) ? "" : "[不可播放] ") + w.title))), React.createElement("div", { className: "we-picker__row" }, React.createElement("button", {
        className: "we-picker__btn", type: "button",
        onClick: onTogglePlay, disabled: !sel.url,
    }, sel.playing ? "暂停" : "播放"), React.createElement("button", {
        className: "we-picker__btn", type: "button",
    ...
    React.createElement("div", { className: "we-picker__row we-picker__playlist-row" }, React.createElement("span", { className: "we-picker__hint we-picker__label" }, "轮播列表"), React.createElement("select", {
        className: "we-picker__playlist-select",
        value: sel.rotationGroupId,
        onChange: onGroupChange,
        disabled: groups.length === 0,
    }, React.createElement("option", { value: "" }, groups.length ? "— 选择轮播列表 —" : "— 暂无轮播列表 —"), ...groups.map((g) => React.createElement("option", {
        key: g.id, value: g.id,
    }, g.name + "（" + groupWallpapers(g).length + " 可播放 · " + g.interval + " 分钟）"))), React.createElement("button", {
        className: "we-picker__btn", type: "button",
        ...
        disabled: !sel.rotationGroupId,
    }, "删除")), editing && React.createElement("div", { className: "we-picker__editor" }, React.createElement("div", { className: "we-picker__row" }, React.createElement("span", { className: "we-picker__hint we-picker__label" }, "名称"), React.createElement("input", {
        className: "we-picker__text", type: "text",
        value: editing.name,
        onInput: (e) => { editing.name = e.target.value; emit(); },
    })), React.createElement("div", { className: "we-picker__row" }, React.createElement("span", { className: "we-picker__hint we-picker__label" }, "间隔"), React.createElement("select", {
    ...
    }, ...INTERVALS.map((minutes) => React.createElement("option", { key: minutes, value: String(minutes) }, minutes + " 分钟"))), React.createElement("span", { className: "we-picker__hint we-picker__label" }, "顺序"), React.createElement("select", {
        className: "we-picker__playlist-select",
        value: editing.order,
        onChange: (e) => { editing.order = e.target.value; emit(); },
    }, React.createElement("option", { value: "sequence" }, "顺序"), React.createElement("option", { value: "random" }, "随机"))), React.createElement("div", { className: "we-picker__editor-list" }, playableInventory().length === 0
        ...
        : playableInventory().map((w) => React.createElement("label", {
            ...
            onChange: () => {
                ...
                if (i >= 0)
                    editing.wallpaperIds.splice(i, 1);
                else
                    editing.wallpaperIds.push(w.id);
                emit();
            },
        }), w.title))), React.createElement("div", { className: "we-picker__row" }, React.createElement("span", { className: "we-picker__hint" }, "已选 " + editing.wallpaperIds.length + " 个"), sel.inventory.playlists.length > 0 && React.createElement("select", {
        className: "we-picker__playlist-select",
        value: "",
        onChange: (e) => {
            const p = sel.inventory.playlists.find((pl) => pl.id === e.target.value);
            if (p)
                importPlaylistIntoDraft(p);
        },
    }, React.createElement("option", { value: "" }, "从 WE 播放列表导入…"), ...sel.inventory.playlists.map((p) => React.createElement("option", {
        key: p.id, value: p.id,
    }, p.name + "（" + (p.portableCount || 0) + " 可播放）")))), React.createElement("div", { className: "we-picker__row" }, React.createElement("button", {
        className: "we-picker__btn", type: "button",
        onClick: saveEditingGroup,
    }, "保存"), React.createElement("button", {
        className: "we-picker__btn", type: "button",
        onClick: cancelEditGroup,
    }, "取消"))), React.createElement("div", { className: "we-picker__row we-picker__rotation-row" }, React.createElement("label", { className: "we-picker__rotation-toggle" }, React.createElement("input", {
        type: "checkbox",
        checked: sel.rotationEnabled,
        onChange: onToggleRotation,
        disabled: !sel.rotationGroupId || playableCount < 2,
    }), "自动轮转"), React.createElement("select", {
        className: "we-picker__rotation-interval",
        ...
        title: "当前列表的切换间隔",
    }, ...INTERVALS.map((minutes) => React.createElement("option", { key: minutes, value: String(minutes) }, minutes + " 分钟"))), !sel.rotationGroupId && React.createElement("span", { className: "we-picker__hint" }, "请先选择或新建一个轮播列表"), sel.rotationGroupId && playableCount < 2 && React.createElement("span", { className: "we-picker__hint" }, "当前列表至少需要 2 个可播放壁纸")), sel.id && React.createElement(React.Fragment, null, SliderRow("壁纸模糊", 0, 60, 1, sel.wallpaperBlur, onWallpaperBlur, sel.wallpaperBlur + "px"), SliderRow("暗化", 0, 90, 5, Math.round(sel.scrim * 100), onScrim, Math.round(sel.scrim * 100) + "%"), SliderRow("边框", 0, 90, 5, Math.round(sel.border * 100), onBorder, Math.round(sel.border * 100) + "%"), SliderRow("玻璃", 0, 40, 1, sel.blur, onBlur, sel.blur + "px")), React.createElement("div", { className: "we-picker__row" }, React.createElement("span", { className: "we-picker__hint" }, (group
...
//# sourceMappingURL=wallpaper-layer.js.map