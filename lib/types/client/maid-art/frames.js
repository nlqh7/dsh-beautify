import { FRAME_ART, FRAME_IDS } from "./frame-art.generated.js";
...
function cssModuleClass(document, moduleId, exportName) {
    const style = Array.from(document.querySelectorAll('style[data-plugin-css]'))
...
function messageTargets(body) {
    ...
    const userMessages = userBubbleClass
        ? Array.from(body.getElementsByClassName(userBubbleClass)).filter((target) => target instanceof HTMLElement)
        : [];
    const assistantMessages = assistantRootClass && assistantBodyClass
        ...
        : [];
    return [
        ...userMessages.map((target) => ({ target, role: 'user' })),
        ...assistantMessages.map((target) => ({ target, role: 'assistant' })),
...
function visibleCandidates(body, id) {
    return Array.from(body.querySelectorAll(SELECTORS[id]))
        .filter((target) => isVisible(target, body));
}
function hasAccessibleName(button) {
    return Boolean(button.getAttribute('aria-label')?.trim()
...
function closestBorderedAncestor(target, body) {
    for (let current = target.parentElement; current && current !== body; current = current.parentElement) {
        if (isVisible(current, body) && hasRenderedBorder(current, body))
            return current;
    }
    return null;
}
...
export function createFrameController(body) {
    let disposed = false;
    let frame;
    let mode = 'light';
    ...
    const sync = () => {
        ...
        targets.get('composer')?.forEach((target) => {
            ...
            if (shell)
                desired.set(shell, { frame: 'composer-shell' });
        });
