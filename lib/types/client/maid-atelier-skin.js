/**
 * Deep-sea maid atelier skin (深海女仆工坊), ported from
 * @linxin666/dsh-client-ui-skin-maid-atelier (CC BY-NC-SA 4.0 — attribution
 * chain in NOTICE). Controller-style: initMaidAtelier() mounts the layered
 * background, collapsible sidebar artwork, and ornamental chrome; dispose()
 * restores every skin-owned write.
 */
import { MAID_ATELIER_BOW_CLEAN, MAID_ATELIER_CHIBI, MAID_ATELIER_ICON, MAID_ATELIER_NEW_SESSION, MAID_ATELIER_SIDEBAR_SWAG, MAID_ATELIER_TOP_TRIM_TILE, } from "./maid-art/maid-atelier-art.js";
import { MAID_ATELIER_MAID_LEFT, MAID_ATELIER_MAID_RIGHT, MAID_ATELIER_PALACE_DARK, MAID_ATELIER_PALACE_LIGHT, } from "./maid-art/maid-atelier-background-art.generated.js";
import { MAID_ATELIER_BOTTOM_CREST, MAID_ATELIER_BOTTOM_TRIM_TILE, MAID_ATELIER_COMPOSER_FRAME, MAID_ATELIER_SETTINGS_FRAME, MAID_ATELIER_SIDEBAR_CORNER, } from "./maid-art/maid-atelier-chrome-art.generated.js";
import { MAID_ATELIER_WORKSPACE_RIBBON, MAID_ATELIER_WORKSPACE_SHIELD, } from "./maid-art/maid-atelier-workspace-art.generated.js";
import { MAID_ATELIER_CSS } from "./maid-atelier-css.js";
...
function createBodyAttributeLease(body, attribute, value = '') {
    ...
    return {
        ...
        release() {
            ...
            if (state === undefined || !state.owners.delete(owner))
                return;
            ...
            if (attributes?.size === 0)
                bodyAttributeLeases.delete(body);
            if (body.getAttribute(attribute) !== state.value)
                return;
            if (state.originalValue === null)
                body.removeAttribute(attribute);
            else
                body.setAttribute(attribute, state.originalValue);
        },
    };
}
...
function decorateTitlebarBrand(ownedNodes) {
    ...
    if (!titlebar)
        return;
    if (titlebar.querySelector("[data-skin-chrome='titlebar-brand']"))
        return;
...
function decorateWorkspaceTree(decoratedElements) {
    ...
    if (!sidebar)
        return;
    sidebar.querySelectorAll('[data-maid-workspace-group], [data-maid-workspace-row], [data-maid-workspace-active], [data-maid-session-row], [data-maid-session-flat], [data-maid-session-first], [data-maid-session-last]').forEach((element) => {
    ...
    sidebar.querySelectorAll("[role='tree']").forEach((tree) => {
        ...
        const decorateGroup = () => {
            ...
            if (sessionRows[0])
                sessionRows[0].dataset.maidSessionFirst = '';
            if (sessionRows.at(-1))
                sessionRows.at(-1).dataset.maidSessionLast = '';
            ...
            workspaceRow.toggleAttribute('data-maid-workspace-active', containsCurrent);
        };
        rows.forEach((row) => {
...
/**
 * Mount the maid-atelier layered background and chrome. Returns a controller
 * whose dispose() retracts every DOM and CSS write.
 */
export function initMaidAtelier() {
    ...
    const dispose = () => {
        ...
        if (composerMotionTimer !== undefined)
            clearTimeout(composerMotionTimer);
        ...
        if (railSearchFocusFrame !== undefined)
            cancelAnimationFrame(railSearchFocusFrame);
    ...
    if (document.querySelector(`style[data-plugin-css=${JSON.stringify(MAID_ATELIER_CSS_TAG)}]`) === null) {
        const tag = document.createElement('style');
        tag.dataset.plugin = 'dsh-beautify';
        tag.dataset.pluginCss = MAID_ATELIER_CSS_TAG;
        tag.textContent = MAID_ATELIER_CSS;
    ...
    if (!hasAcceleratedWebGL())
        lowPowerLease.acquire();
    ...
    const syncProjectedState = () => {
        ...
        set(PROJECTED_STATE_ATTRIBUTES.betterSidebarOpen, document.querySelector(BETTER_SIDEBAR_SELECTOR) !== null
    ...
    recoverRailSearchFocus = (event) => {
        ...
        if (target === null || railSearch == null
            || railSearch.querySelector("input[class*='searchInput']") !== null)
            return;
        if (railSearchFocusFrame !== undefined)
            cancelAnimationFrame(railSearchFocusFrame);
    ...
    const syncSettingsBackdropFrame = () => {
        const expanded = document.querySelector(`${SETTINGS_TRIGGER_SELECTOR}[aria-expanded='true']`);
    ...
    const syncSidebarDecorations = () => {
        ...
        if (sidebar === null)
            clearSidebarWidth();
        else if (resizeObserver === undefined)
    ...
    // rAF-coalesced MutationObserver: opening a large surface (e.g. the settings
    // panel) mutates the subtree in one burst, and scanning every added node
    // synchronously in the microtask made that open visibly janky. Records are
    // queued here and the scan runs ONCE in the next frame — still before paint,
    // so the wide/rail hand-off stays immediate, but a settings-open burst costs
    // a single pass instead of interleaving dozens of blocking scans into the
    // app's own render. Skin-owned insertions are ignored so decorating a
    // React-owned node cannot schedule a redundant whole-sidebar pass.
    let observerFrame = 0;
    let observerRecords = [];
    const processObserverRecords = (records) => {
        ...
        for (const record of records) {
            ...
            if (target?.closest(TERMINAL_SELECTOR) !== null)
                continue;
        ...
        if (projectedStateChanged)
            syncProjectedState();
        if (sidebarStructureChanged)
            syncSidebarDecorations();
    ...
    observer = new MutationObserver((records) => {
        ...
        observerFrame = requestAnimationFrame(() => {
            observerFrame = 0;
            const batch = observerRecords;
            observerRecords = [];
            processObserverRecords(batch);
        });
    });
    ...
    return {
        ...
        setWide: () => {
            // Sidebar width follows the host via ResizeObserver; nothing to drive.
        },
        dispose,
    };
}
//# sourceMappingURL=maid-atelier-skin.js.map