...
import { LiquidGlassLayer } from "./liquid-glass/theme-layer.js";
import { LIQUID_GLASS_LEVELS } from "./liquid-glass/settings-store.js";
...
function normalizeLevel(v) {
    if (v === 'lite' || v === 'standard' || v === 'ultra')
        return v;
    if (v === '1')
        return 'standard'; // 兼容旧开启标记
    return 'off';
}
function initLiquidGlass(ctx) {
    let layer = null;
    const applyLevel = (level) => {
        try {
            ...
            if (layer === null)
                layer = new LiquidGlassLayer(ctx);
    ...
    let level = 'off';
    try {
        level = normalizeLevel(localStorage.getItem(LIQUID_GLASS_STORAGE_KEY));
    }
    catch { /* noop */ }
    applyLevel(level);
    const onToggle = (e) => {
        ...
        try {
            localStorage.setItem(LIQUID_GLASS_STORAGE_KEY, next);
        }
        catch { /* noop */ }
        applyLevel(next);
    };
