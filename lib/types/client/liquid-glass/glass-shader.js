...
`;
function createLiquidGlassShader(canvas, currentOpts) {
    ...
    function loadWallpaper(url) {
        ...
        let cleanUrl = String(url).replace(/^(video:)+/, '');
        let posterUrl = '';
        ...
        try {
            ...
            if (sidebarEl) {
                ...
                const classStr = typeof rootEl.className === 'string'
                    ? rootEl.className
                    : (typeof rootEl.className?.baseVal === 'string' ? rootEl.className.baseVal : '');
