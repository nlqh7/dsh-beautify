 ...
 */
import { cursorSkinFor } from "./cursor-images.js";
...
function isLinkTarget(target) {
    if (!(target instanceof HTMLElement))
        return false;
    return target.closest('a[href], button, [role="button"], [role="link"], input[type="submit"], input[type="button"], [data-cursor="pointer"]') !== null;
}
 ...
 */
export function initWhaleCursor(gated, skinId = 'whale', size = 48, staticState = true) {
    ...
    let stateOverrides = {};
    const isStateEnabled = (state) => stateOverrides[state] === true || (stateOverrides[state] !== false && !DISABLED_BY_DEFAULT.has(state));
    ...
    // while the pointer pets the whale, the cursor presents the hand (link) art.
    ...
    const setGate = () => {
        ...
        // whale gate must close while that state is active.
        const stateOk = isStateEnabled(current);
        const nextEnabled = imageReady && placed && gated() && !reducedMotion && stateOk;
    ...
    const onMove = (event) => {
        ...
        if (!enabled)
            return;
        // State detection walks up to four `closest()` queries, which on a fast
        // pointer sweep over dense DOM would dominate the handler. In static mode
        // the sprite stays on the regular form and detection is skipped entirely.
        // While dragging, the drag state must not be overwritten by target
        // detection, so detection pauses too. Petting the whale (the widget sets
        // html[data-dsh-whale-hover]) presents the hand/link art.
        if (!staticMode && !dragging) {
            ...
            if (event.target !== stateTarget || whaleHover !== lastWhaleHover) {
                ...
                const state = explicit ?? (whaleHover ? 'link' : stateForTarget(event.target));
                if (state !== current)
                    applyState(state);
            }
        }
...
//# sourceMappingURL=whale-cursor.js.map