...
function steamPathFromRegistry() {
    ...
    try {
        ...
        const out = execFileSync(reg, ['query', 'HKCU\\Software\\Valve\\Steam', '/v', 'SteamPath'], { encoding: 'utf8', windowsHide: true, timeout: 5000, stdio: ['ignore', 'pipe', 'ignore'] });
...
function locateWallpaperEngine() {
    ...
    for (const raw of candidates) {
        ...
        if (seen.has(dir))
            continue;
        seen.add(dir);
        if (existsSync(join(dir, 'wallpaper32.exe')))
            return dir;
    }
    return null;
}
...
function owningLibraries() {
    ...
    for (const probe of steamProbeDirs()) {
        ...
        if (existsSync(join(probe, 'steamapps', 'common', 'wallpaper_engine')))
            libs.push(probe);
    }
    return [...new Set(libs)];
}
function inferType(file) {
    if (/\.(mp4|webm|mkv|avi|mov)$/i.test(file))
        return 'video';
...
function playlistRows(profile) {
    ...
    if (!general || typeof general !== 'object')
        return [];
    if (Array.isArray(general.playlists) && general.playlists.length)
        return general.playlists;
    ...
    function serveFile(absPath, req, res) {
        ...
        if (range) {
            ...
            if (Number.isNaN(start))
                start = 0;
            if (Number.isNaN(end) || end >= st.size)
                end = st.size - 1;
            if (start > end) {
                res.statusCode = 416;
    ...
    function resolveMediaToken(token) {
        ...
        if (!rel) {
            ...
            if (existsSync(pj)) {
                try {
                    const o = JSON.parse(readFileSync(pj, 'utf8'));
                    if (o && typeof o.file === 'string' && o.file.trim())
                        entry = o.file;
                }
                catch { /* keep default */ }
            }
        ...
        if (norm === base || norm.toLowerCase().startsWith(base.toLowerCase() + '\\'))
            return norm;
        return null;
    }
...
//# sourceMappingURL=wallpaper-engine.js.map